...
const divider = (numbersArray) => {
  returnArr = [];
  returnArr[0] = numbersArray.filter((v) => {return !(v % 2)});
  returnArr[1] = numbersArray.filter((v) => {return v % 2});
  return returnArr;
...
const both = (arr1, arr2) => {
  return arr1.filter((v) => {
      return arr2.includes(v) ? true : false;
  })
}
...
users.push(user1);
users.push({name: 'derp', email: 'derp@derp.com', password: 'derpderpderp', username: 'derpest'});
users.push({name: 'kerp', email: 'kerp@kerp.com', password: 'kerpderpderp', username: 'kerpest'});
